...
- **`briefs` are optional input.** Node 1 accepts `briefs` from state (declared in `RunState` for exactly this). When omitted the run starts with an empty brief set and node 1's own behaviour applies — the API never fabricates briefs.
...
- **Cap policy is a refusal, not a warning.** The cap is a stated invariant (AGENTS.md §3); the remedy is to raise it in Agents → budget caps, so a refusal is actionable rather than a silent overspend.
