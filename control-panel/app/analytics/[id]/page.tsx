    ...
    <AppShell>
      <div className="space-y-4">
        ...
        {error ? (
          ...
          <p className="font-mono text-xs text-muted-foreground">Loading KPI…</p>
        ) : (
          <>
            {rec && <RetirementBanner rec={rec} />}
            <KpiSeriesTable kpi={kpi} />
            <p className="font-mono text-[11px] text-muted-foreground">
              Thresholds: min_units={kpi.kpi.thresholds.min_units ?? "—"} · eval_window=
              {kpi.kpi.thresholds.eval_window_days ?? "—"}d
            </p>
          </>
        )}
      </div>
